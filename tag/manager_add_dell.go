...
func (obj *TagManager) AddBasicTags(ctx context.Context, tags []string, targetId string, owner string, info string) error {
	for _, v := range tags {
		Debug(ctx, "tag-:"+v+":-")
		if v == "" {
			continue
		}
		obj.AddBasicTag(ctx, v, targetId, owner, info)
	}
	return nil
}
